...
documents, with citations. **Branch: `security` (stage 2 of 3)** — the RAG
pipeline plus the reliability & safety layer. Every model call is routed
in-process by a LiteLLM Router (Gemini primary, Groq fallback —
`hr_assistant/llm.py` (12), doc 14). Deployment (Docker / Cloud Run / OAuth)
is the `deployment` branch, not here. Full explanation: `docs/01`-`docs/16`.
Fast orientation: `summary.md`. This skill is the "before you touch the
code" brief.
...
The app (`app.py` / `main.py`) uses the CLEAN `hr_policies` collection with
the guarded tool: the category filter is close to a no-op there (all docs
are HR), but the relevance floor still turns a weak match into "I don't
have that". `demo_reliability.py` / `redteam_test.py` use the noisy one.
