"""Data ingestion — the one place documents get loaded into Qdrant.

Flow:

    local data/ files
        -> GCS  raw/hr-policies/   +  raw/other-data/
        -> GCS  processed/...      (pdf/docx/pptx parsed to JSON, once)
        -> Qdrant collections      (chunked + embedded + upserted)

Idempotent by default: a Qdrant collection that already has points in it is
left alone. Pass force=True to rebuild it from scratch.

`ingest.py` at the repo root is the runnable entry point. main.py / app.py
/ evaluate.py never ingest — they call vector_store.load_vector_store() to
connect to what this module already built.
"""
...
def ingest_hr_policies(force: bool = False) -> None:
    """The clean HR-only collection read by main.py / app.py / evaluate.py."""
...
def ingest_noisy_corpus(force: bool = False) -> None:
    """The mixed HR + noise collection used by demo_reliability.py / redteam_test.py.
