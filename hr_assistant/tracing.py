...
tracing is turned on, automatically sends a trace of every LLM call, tool
call, and agent step to your LangSmith project.

This module doesn't turn tracing on — the env vars already do that. All it
does is log, once per run, whether tracing is active, so it's obvious from
the logs whether this run was traced.
"""
